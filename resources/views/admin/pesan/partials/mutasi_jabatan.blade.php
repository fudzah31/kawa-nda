{{-- resources/views/admin/pesan/partials/mutasi_jabatan.blade.php --}}
<div class="space-y-8">
    ...
    <div class="overflow-hidden rounded-xl border">
      <table class="w-full text-sm text-gray-700">
        <tbody class="divide-y divide-gray-200">
          ...
          <tr>
            <th class="px-4 py-2 font-medium text-gray-600">Jabatan Saat Ini</th>
      ...
      <li>
        ...
        @if(!empty($data->sk_pelantikan))
          <a href="{{ asset('storage/'.$data->sk_pelantikan) }}" target="_blank"
             class="ml-2 inline-block text-sm px-3 py-1 rounded-lg bg-blue-100 text-blue-700 hover:bg-blue-200 transition">
             📄 Lihat Dokumen
          </a>
        @else
  ...
  <div class="bg-white p-6 rounded-2xl shadow-md border flex justify-between items-center">
    ...
    <form action="{{ route('admin.pesan.update', $data->id) }}" method="POST" class="flex items-center gap-3">
      @csrf
      @method('PUT')
      <input type="hidden" name="tipe" value="mutasi_jabatan">
